...
vec4 CompositeBottomAndTop(vec4 bottom, float bottomAlpha, vec4 top, float topAlpha)
{
	...
	returnMe.rgb = satRGBToHSL(darkenedBottom.rgb);
	//	this blend mode uses top's S with bottom's H and L (requires converting top from RGB->HSL)
	returnMe.g = satRGBToHSL(top.rgb).g;
